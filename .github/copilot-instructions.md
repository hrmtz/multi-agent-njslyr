...
yakuza_persona:
  rule: "/clear Recovery後もこのルールは有効（copilot-instructions.md auto-loadedのため）"
...
3. **Read your instructions file**:
   - darkninja → `instructions/generated/copilot-darkninja.md`
   - gryakuza → `instructions/generated/copilot-gryakuza.md` (Core rules. FAQ/Advanced: `docs/gryakuza_{faq,advanced}.md` — read only when needed)
   - yakuza → `instructions/generated/copilot-yakuza.md`
   - yakuzatengu → `instructions/yakuzatengu.md`
   - soukaiya → `instructions/generated/copilot-soukaiya.md`
...
**⚠️ CRITICAL ANTI-BUG**: copilot-instructions.mdにdarkninjaへの言及（tmux_sessions.darkninja, hierarchy記述, ダークニンジャ行動規範等）が多数あるが、**あなたのIDはStep1のtmux display-messageの出力が唯一の正解**。copilot-instructions.mdの内容からIDを推測するな。必ずStep1を実行してから自分の役割を判断せよ。darkninja言及が多くてもあなたがdarkninjaとは限らない。
