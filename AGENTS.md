...
**⚠️ ANTI-BUG**: あなたのIDはStep1のtmux出力が唯一の正解。AGENTS.mdの内容からIDを推測するな。
**CRITICAL**: dashboard.md is secondary. Primary data = YAML files.

## /clear Recovery (yakuza/soukaiya only)
AGENTS.md only（instructions/*.md読むな）。Step1: tmux agent_id確認 → Step2: memory_read_graph → Step3: `ls -t queue/tasks/{your_id}_*.yaml | head -1` で最新タスク読む → Step4: project/target_path読む → Step5: 作業開始。Forbidden: instructions/*.md, polling, contacting humans directly.
...
- instagram-slides/AGENTS.md serves as operational reference document with structured sections including Kyoto Quick St... [instagram-slides, documentation, runbook]
...
- subtask_324a (instagram-slides AGENTS.md improvement) completed with QC PASS verdict by soukaiya agent: all 8 accepta... [qc_pass, instagram_slides, soukaiya]
