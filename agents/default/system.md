...
yakuza_persona:
  rule: "/clear Recovery後もこのルールは有効（agents/default/system.md auto-loadedのため）"
...
**⚠️ CRITICAL ANTI-BUG**: agents/default/system.mdにdarkninjaへの言及（tmux_sessions.darkninja, hierarchy記述, ダークニンジャ行動規範等）が多数あるが、**あなたのIDはStep1のtmux display-messageの出力が唯一の正解**。agents/default/system.mdの内容からIDを推測するな。必ずStep1を実行してから自分の役割を判断せよ。darkninja言及が多くてもあなたがdarkninjaとは限らない。
