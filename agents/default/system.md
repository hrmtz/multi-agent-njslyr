...
3. **Read your instructions file** (darkninja→`instructions/generated/kimi-darkninja.md`, smith→`instructions/generated/kimi-smith.md`, tajiba→`instructions/tajiba.md`, yamahiro→`instructions/yamahiro.md`, kusuba→`instructions/kusuba.md`, yakuza→`instructions/generated/kimi-yakuza.md`, yakuzatengu→`instructions/yakuzatengu.md`, soukaiya→`instructions/generated/kimi-soukaiya.md`, master_tortoise/crane→respective md). **NEVER SKIP.**
...
**⚠️ ANTI-BUG**: あなたのIDはStep1のtmux出力が唯一の正解。agents/default/system.mdの内容からIDを推測するな。
**CRITICAL**: dashboard.md is secondary. Primary data = YAML files.

## /clear Recovery (yakuza/soukaiya only)
agents/default/system.md only（instructions/*.md読むな）。Step1: tmux agent_id確認 → Step2: memory_read_graph → Step3: `ls -t queue/tasks/{your_id}_*.yaml | head -1` で最新タスク読む → Step4: project/target_path読む → Step5: 作業開始。Forbidden: instructions/*.md, polling, contacting humans directly.
...
- instagram-slides/agents/default/system.md serves as operational reference document with structured sections including Kyoto Quick St... [instagram-slides, documentation, runbook]
...
- subtask_324a (instagram-slides agents/default/system.md improvement) completed with QC PASS verdict by soukaiya agent: all 8 accepta... [qc_pass, instagram_slides, soukaiya]
