...
#### ⚠️ 既知の失敗パターンと対処（2026-03-02 実測）

**失敗パターン1: inbox処理後にloopへ戻らない**
- 症状: surikenを受信 → inbox確認・報告 → `❯` prompt待ちで停止 → ハートビート未送信
- 原因: inbox処理を「サイクル1回分」と誤認し、sleep 60を実行せず終了する
- 対処: **inboxが0件でも1件でも、必ずサイクル全手順（heartbeat含む）を完了してからsleep 60**

**失敗パターン2: ハートビートをサイクル末尾に忘れる**
- 症状: check_incident_triggers → analyze → report → sleep 60（heartbeat飛ばし）
- 対処: `send_heartbeat`はsleep 60の**直前**に必ず実行。順番: ...→ send_heartbeat → sleep 60

**失敗パターン3: エラー発生時にループ中断**
- 症状: ツール呼び出しエラー → エラー出力して停止
- 対処: エラーはログに記録してループを継続。`try/catch`相当の意識で書く。
  「エラーが出た = サイクル終了」ではない。「エラーが出た = 記録して次サイクルへ」

**鉄則**: どんな状況でも、サイクルの最後の行は必ず `Bash sleep 60` ツールコールである。
...
- チャネル: SSH経由（対向マシンの queue/heartbeat/crane.yaml を直接更新）
- 間隔: 60秒
...
if [[ -n "$PEER_HOST" && -n "$PEER_PROJECT_ROOT" ]]; then
    ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=accept-new -o BatchMode=yes "$PEER_HOST" \
        ...
        echo "[crane] WARNING: SSH heartbeat failed" >&2
fi
```
