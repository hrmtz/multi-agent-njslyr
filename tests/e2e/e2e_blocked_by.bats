...
@test "E2E-007-A: task B waits for task A to complete before starting" {
    ...
    # 1. Place task A for yakuza1 (no blocked_by)
    cp "$PROJECT_ROOT/tests/e2e/fixtures/task_yakuza1_basic.yaml" \
       "$E2E_QUEUE/queue/tasks/yakuza1.yaml"

    # 2. Place task B for yakuza2 (blocked_by: subtask_test_001a)
    cp "$PROJECT_ROOT/tests/e2e/fixtures/task_yakuza2_blocked.yaml" \
    ...
    sleep 2
    assert_yaml_field "$E2E_QUEUE/queue/tasks/yakuza2.yaml" "task.status" "blocked"
...
os.replace(tmp_path, '$E2E_QUEUE/queue/tasks/yakuza2.yaml')
"
...
@test "E2E-007-B: blocked task ignores inbox nudge until unblocked" {
    ...
    # 1. Place blocked task for yakuza2
    cp "$PROJECT_ROOT/tests/e2e/fixtures/task_yakuza2_blocked.yaml" \
       "$E2E_QUEUE/queue/tasks/yakuza2.yaml"
    ...
    [ ! -f "$E2E_QUEUE/queue/reports/yakuza2_report.yaml" ]
}
