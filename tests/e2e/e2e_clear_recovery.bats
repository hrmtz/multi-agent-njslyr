...
@test "E2E-003-A: /clear with assigned task triggers auto-recovery" {
    # 1. Place task YAML for yakuza1 (status: assigned)
    cp "$PROJECT_ROOT/tests/e2e/fixtures/task_yakuza1_basic.yaml" \
       "$E2E_QUEUE/queue/tasks/yakuza1.yaml"

    # 2. Send /clear to yakuza1 (not inbox nudge — direct /clear)
    local yakuza1_pane
    yakuza1_pane=$(pane_target 1)
    send_to_pane "$yakuza1_pane" "/clear"
...
@test "E2E-003-B: /clear without assigned task does not crash" {
    # 1. No task YAML placed — yakuza1 has nothing to do
    ...
    [ ! -f "$E2E_QUEUE/queue/reports/yakuza1_report.yaml" ]

    # 5. Verify mock is still alive — send test input, check it's processed
    send_to_pane "$yakuza1_pane" "health_check"
    run wait_for_pane_text "$yakuza1_pane" "Processed input: health_check" 10
    assert_success
}
